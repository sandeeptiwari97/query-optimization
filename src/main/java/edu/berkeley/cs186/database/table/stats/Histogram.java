...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
    ...
    // TODO: HW4 implement
    Iterator<Record> rIter = table.iterator();
    //1. first calculate the min and the max values
    while (rIter.hasNext()) {
      Record r = rIter.next();
      float val = quantization(r, attribute);
      if (minValue >= val) {
        minValue = val;
      }
      if (maxValue <= val) {
    ...
    if (width == 0) {
      for (int i = 0; i < numBuckets; i++) {
        buckets[i] = new Bucket<>(minValue, maxValue);
      }
      Iterator<Record> rIter1 = table.iterator();
      while (rIter1.hasNext()) {
        Record r = rIter1.next();
        float val = quantization(r, attribute);
        get(numBuckets - 1).increment(val);
      }
    } else {
      for (int i = 0; i < numBuckets; i++) {
        if (i < numBuckets - 1) {
          buckets[i] = new Bucket<Float>(i*width, (i+1)*width);
        } else {
          buckets[i] = new Bucket<Float>(maxValue-width, maxValue+1);
        }
      }
...
//      rIter = table.iterator();
      while (rIter1.hasNext()) {
        Record r = rIter1.next();
        float val = quantization(r, attribute);
        int index = bucketIndex(val);
        get(index).increment(val);
      }
    }
  }
  ...
  private float [] allEquality(float qvalue){
    ...
    // TODO: HW4 implement;
    int target = bucketIndex(qvalue);
    for (int i = 0; i < numBuckets; i++) {
      if (i == target) {
        result[i] = (1 / (float) get(i).getDistinctCount());
      } else {
        result[i] = 0;
      }
    }
 ...
 /**
  ...
  private float [] allNotEquality(float qvalue){
    ...
    for (int i = 0; i < numBuckets; i++) {
      if (i == target) {
        result[i] = (1 - (1 / (float) get(i).getDistinctCount()));
      } else {
        result[i] = 1;
      }
    }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    for (int i = 0; i < numBuckets; i++) {
      if (i < target) {
        ...
        result[i] = (get(i).getEnd() - qvalue) / width;
      }
    }
     return result;
  }
  ...
  private float [] allLessThan(float qvalue){
    ...
    for (int i = 0; i < numBuckets; i++) {
      if (i < target) {
       ...
       result[i] = (qvalue - get(i).getStart()) / width;
      }
    }
     return result;
  }
